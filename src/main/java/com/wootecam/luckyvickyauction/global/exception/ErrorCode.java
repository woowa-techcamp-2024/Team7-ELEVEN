...
public enum ErrorCode {
    ...
    A008("경매 생성 시, 경매 지속시간은 10, 20, 30, 40, 50, 60분이 아닌 경우 예외가 발생합니다."),
    A009("경매 가격 변동폭은 경매 가격보다 낮아야 합니다."),
    A010("경매 생성 시, 할인율이 0퍼센트 미만이거나 50 퍼센트를 초과한 경우 예외가 발생합니다."),
    ...
    A020("경매 생성 시, 경매 시작 시간이 현재 시간보다 이른 경우 예외가 발생합니다."),
    A021("경매 재고 변경 시, 경매가 준비중인 상태가 아닐때 예외가 발생합니다."),
    A022("경매 재고 변경 시, 추가 및 삭제하는 재고량이 1 미만일 경우 예외가 발생합니다."),
    A023("경매 재고 변경 시, 변경 후의 재고가 원래 재고보다 많을 경우 예외가 발생합니다."),
    A024("경매 취소 시, 사용자 역할이 판매자가 아닌 경우 예외가 발생합니다."),
    A025("경매 취소 시, 경매를 생성한 판매자와 경매 취소 요청 사용자가 다를 경우 예외가 발생합니다."),
    A026("경매 취소 시, 경매가 준비중인 상태가 아닐때 예외가 발생합니다."),
    A027("경매 상세 조회 시, 요청한 판매자가 경매의 판매자가 아닐 경우 예외가 발생합니다."),
    A028("경매 생성 시, 가격 변동 정책이 적용된 경매에서 최대 할인가를 적용했을떄 최소 가격 이하로 떨어지는 경우 예외가 발생합니다."),
    A029("경매 입찰 진행 시, 현재 상품의 가격과 사용자가 구매 요청한 가격이 다를때 예외가 발생합니다."),
    A030("가격 정책 객체를 JSON으로 변환 시, 변환이 불가능할 경우 예외가 발생합니다."),
    A031("JSON을 가격 정책 객체로 변환 시, 타입이 올바르지 않을 경우 예외가 발생합니다."),
    A032("JSON을 가격 정책 객체로 변환 시, 변환이 불가능할 경우 예외가 발생합니다."),
    A033("경매 입찰 요청 시, 요청 가격이 0보다 작으면 예외가 발생합니다."),
    A034("경매 입찰 요청 시, 요청 수량이 1 미만일 경우 예외가 발생합니다."),
    A035("단일 경매 조회 시, 경매가 존재하지 않는 경우 예외가 발생합니다."),
    ...
    M006("회원 가입 시, 비밀번호가 빈칸 또는 공백인 경우 예외가 발생합니다."),
    M007("회원 가입 시, 비밀번호는 8자 이상 20자 이하가 아닌 경우 예외가 발생합니다."),
    M008("회원 가입 시, 비밀번호에 숫자가 포함되어 있지 않은 경우 예외가 발생합니다."),
    M009("회원 가입 시, 비밀번호에 알파벳 소문자가 포함되어 있지 않은 경우 예외가 발생합니다."),
    M010("회원 가입 시, 비밀번호에 영문자와 숫자 외에 다른 문자가 포함되어 있는 경우 예외가 발생합니다."),
